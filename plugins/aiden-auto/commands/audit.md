...
# 자동 구현 (개선 제안 자동 적용)
/audit --auto-implement     # 점검 + 자동 구현 (T1 즉시적용, T2 PR, T3 백로그)
/audit ledger               # 개선 이력 조회
/audit ledger stats         # 개선 통계 요약
...
/audit 실행
    ...
    │       ├─ [2/5] Lead 직접 WebSearch 호출 (3-tier 쿼리, 8개)
    │       │       ├─ 병렬 호출 권장 (단일 턴 내 여러 WebSearch 블록)
    │       │       └─ 결과 없으면: "관련 아티클 없음" 후 Phase 3으로
    │       ├─ [3/5] Lead 직접 갭 분석 (아티클 vs 인벤토리)
    │       │       └─ 3분류: 이미 구현 / 부분 구현 / 미구현 + 복잡도 태그
    │       ├─ [4/5] 개선 아이디어 출력
    ...
    │       └─ 전체 건강도 점수
    │
    └─ [--auto-implement 시 추가 Phase]
            │
            ├─ [Phase 0] Impact Check (Phase 1 앞에 실행)
            │       ├─ .claude/audit/improvement-ledger.json 읽기
            │       ├─ 어제 적용 항목 필터 → 파일 상태 검증
            │       └─ 회귀 감지 시 자동 revert + ledger 업데이트
            │
            ├─ [Phase 3.5] Auto-Implement
            │       ├─ Phase 1/1.5/2 액션 아이템 수집
            │       ├─ Safety Tier 분류 (T1/T2/T3)
            │       ├─ T1: 즉시 적용 → git add → 커밋
            │       ├─ T2: audit/improvements 브랜치 → PR 생성
            │       └─ T3: .claude/audit/backlog.md 에 기록
            │
            ├─ [Phase 4] Ledger Update
            │       └─ improvement-ledger.json 에 실행 결과 기록
            │
            └─ [Phase 5] 보고서 강화
                    ├─ 자동 구현 결과 섹션 추가
                    └─ 어제 Impact Check 결과 섹션 추가
```
...
## /audit --auto-implement - 자동 구현 (Self-Improving Loop)

`--auto-implement` 플래그를 사용하면 개선 제안을 자동으로 구현합니다.

### Safety Tier 분류 기준

| Tier | 처리 | 기준 | 예시 |
|:----:|------|------|------|
| T1 | 즉시 자동 적용 + 커밋 | 변경 파일 ≤3, diff ≤50줄, 행위 변경 없음 | 개수 수정, 경로 정규화, doc sync, frontmatter 보강 |
| T2 | 자동 PR 생성 → 사용자 리뷰 | 의미 변경 있으나 범위 한정 | 에이전트 내용 수정, 스킬 구조 변경, 프롬프트 최적화 |
| T3 | 백로그 기록만 | 아키텍처/정책/외부 의존성 | hook 수정, rule 변경, 신규 MCP, 아키텍처 재설계 |

**T1 Deny-list (절대 자동 적용 금지):**
- `.claude/settings.json` hook 정의 변경
- `.claude/rules/*.md` 정책 파일
- `.github/workflows/*.yml` CI/CD 파이프라인
- 파일 삭제 (additive only)
- 외부 명령 필요 작업 (`git pull`, `npm install`)

**Tier 분류 로직 (결정적, LLM 불필요):**
```
IF 파일이 deny-list에 포함 → T3
IF 파일 삭제 포함 → T3
IF 변경 파일 > 3개 → T3
IF 카테고리 ∈ {metadata, doc-sync, path-fix, version, frontmatter} → T1
IF 카테고리 ∈ {agent-content, skill-content, command-content, prompt-optimize} → T2
ELSE → T3
```

### Phase 0: Impact Check (어제 적용 검증)

`--auto-implement` 시 Phase 1 앞에 실행됩니다.

```
Phase 0 실행
    │
    ├─ .claude/audit/improvement-ledger.json 읽기
    ├─ 어제 날짜 entries 필터 (status: "implemented")
    ├─ 각 entry의 files_changed 파일 존재 + 내용 검증
    │   ├─ 정상 → impact_result: "verified"
    │   └─ 회귀 감지 → git revert <commit> → status: "reverted"
    └─ ledger 업데이트
```

### Phase 3.5: Auto-Implement

Phase 1/1.5/2 완료 후 수집된 모든 액션 아이템을 자동 처리합니다.

```
Phase 3.5 실행
    │
    ├─ [1] 액션 아이템 수집
    │       ├─ Phase 1 구조 감사 결과 (⚠️ WARN 항목)
    │       ├─ Phase 1.5 프롬프트 최적화 제안
    │       └─ Phase 2 트렌드 개선 제안 (LOW/MEDIUM)
    │
    ├─ [2] Safety Tier 분류
    │       └─ 각 항목에 T1/T2/T3 태그 부여
    │
    ├─ [3] T1 항목 즉시 적용
    │       ├─ 각 항목별 파일 수정
    │       ├─ 수정 후 검증 (파일 존재, JSON 파싱, frontmatter 무결성)
    │       ├─ 검증 실패 시 → git checkout -- <files> (롤백)
    │       ├─ 검증 통과 시 → git add <files>
    │       └─ 전체 T1 완료 후 단일 커밋:
    │           fix(audit): auto-implement T1 improvements - YYYY-MM-DD
    │
    ├─ [4] T2 항목 PR 생성
    │       ├─ 브랜치: audit/improvements-YYYY-MM-DD
    │       ├─ 각 항목별 파일 수정
    │       └─ gh pr create (모든 T2를 단일 PR로 묶음)
    │
    └─ [5] T3 항목 백로그 기록
            └─ .claude/audit/backlog.md PENDING 섹션에 추가
```

### Phase 4: Ledger Update

모든 처리 결과를 `improvement-ledger.json`에 기록합니다.

```
Phase 4 실행
    │
    ├─ 각 액션 아이템에 대해 entry 생성:
    │   {
    │     "id": "YYYY-MM-DD-NNN",
    │     "date": "YYYY-MM-DD",
    ...
    │     "suggestion": "항목 설명",
    │     "tier": 1|2|3,
    │     "action": "auto-applied|pr-created|backlogged",
    │     "status": "implemented|pending-review|backlogged",
    │     "commit": "해시 (T1만)",
    │     "pr": "#번호 (T2만)",
    │     "files_changed": ["파일 목록"],
    │     "lines_changed": N
...
기존 Phase 3 보고서에 자동 구현 결과 섹션이 추가됩니다.
...
| Tier | 항목 | 상태 | 커밋/PR |
|:----:|------|:----:|---------|
| T1 | CLAUDE.md 경로 수정 | ✅ 적용 | abc1234 |
| T2 | 프롬프트 최적화 3건 | 📋 PR | #150 |
| T3 | hook 구조 개선 | 📝 백로그 | - |
...
| 항목 | 값 |
|------|-----|
| 파일 위치 | `.claude/audit/improvement-ledger.json` |
| 백로그 위치 | `.claude/audit/backlog.md` |
| 형식 | JSON (스키마 v1.0.0) |
| T3 주간 리뷰 | `/audit deep` 실행 시 backlog 승격 검토 |

---

## /audit ledger - 개선 이력 조회

improvement-ledger.json의 내용을 조회합니다.

```bash
/audit ledger               # 최근 10건 이력
/audit ledger stats         # 통계 요약 (총 제안/적용/PR/백로그/revert)
/audit ledger --date 2026-03-30  # 특정 날짜 이력
```
...
| 자동 개선 실행 (매일 스케줄) | `/audit --auto-implement` |
| 개선 이력 확인 | `/audit ledger` |
| 개선 통계 | `/audit ledger stats` |
...
/audit trend 실행
    ...
    │
    ├─ [2/5] 웹 리서치 수행 (Lead 직접 WebSearch)
    │       ├─ 3-tier 검색 쿼리 생성 (Core/Adjacent/Frontier, 총 8개)
    ...
    │       ├─ Lead가 WebSearch 도구를 직접 호출 (8회, 병렬 묶음 권장)
    │       ├─ 각 응답에서 상위 2~3개 아티클 제목 + URL + 요지 추출 (max 15개)
    │       └─ 결과 없으면: "관련 아티클 없음" 출력 후 종료
    │
    ├─ [3/5] 갭 분석 (Lead 직접 수행)
    │       ├─ 아티클 요약 + 워크플로우 인벤토리를 컨텍스트에 로드
    │       ├─ Lead가 3분류 (이미 구현 / 부분 구현 / 미구현) 수행
    │       └─ 각 제안에 복잡도 태그 (LOW: 1파일 / MEDIUM: 2~3파일 / HIGH: 4+파일)
    │
    ├─ [4/5] 개선 아이디어 제안 출력
    │       └─ 구조화된 보고서 (출처 URL 포함)
    │
...
### Phase 2 실행 주체 (2026-04-21 구조 변경)

| 단계 | Before (Agent Teams) | After (Lead 직접) |
|------|---------------------|-------------------|
| 리서치 | researcher 에이전트 (TeamCreate + SendMessage) | Lead가 WebSearch 직접 호출 |
| 갭 분석 | analyst 에이전트 | Lead 컨텍스트 내 3분류 |
| 자동 적용 | executor 에이전트 | Lead가 Edit/Write 직접 (>50줄만 위임) |

**근거**: Agent Teams의 비대화형(isTTY=false) shutdown 실패 (garimto81/claude#140 OPEN). VS Code/비-TTY 환경에서 `shutdown_request` 무응답 → `TeamDelete` 3회 블록 → Phase 2 PARTIAL 반복. Lead 직접 호출로 블로커 제거. 에이전트 위임의 장점(컨텍스트 격리)은 상실하나, 본 작업은 구조화된 쿼리+간단한 분류라 Lead 단독 수행 비용이 낮음.
...
/audit trend --apply 실행
    ...
    │       ├─ Lead 직접 Edit/Write 도구로 파일 수정
    │       │   └─ 50줄 초과 변경 시에만 executor 에이전트 위임 고려
    │       ├─ 적용 완료 후 변경 파일 확인
    │       │   └─ git diff --stat
