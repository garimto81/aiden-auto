...
| 산문 톤 (전문어·내부ID 잔존) | 0개 | user-friendly-reporter 재호출 (max 2회) |
...
3. **다이어그램 = 본문에서 추출 (창작 금지)**: ≤3개 다이어그램은 **확정 본문에 이미 있는 것을 그대로 골라 재사용**한다. 새로 지어내지 않는다 (본문에 없는 그림 = 요약 아님). 시각 비율 ≥50%.
   > ⚠️ **추출은 다이어그램에만 적용**. 산문(Hook/Thesis/결정/Action/결론)은 추출이 아니라 **15세 수준으로 번역**한다 — 원본 PRD 의 전문용어 문장을 *그대로 복사 금지* (복사 = 톤 위반의 근본 원인, 2026-06-05 실측).
4. **결정 ≠ 작업**: 결정 = 의사결정 (선택지 중 하나). 작업 = 진행 단계. 혼동 금지.
5. **Action ≠ 결정**: Action = 사용자가 다음으로 할 일. "확인하기 / 승인하기 / 검토하기" 등.
6. **산문은 15세로 번역 (아래 "응답 스타일 정합" HARD ENFORCE 체크리스트)**: 약어 첫 등장 풀이 / 내부 ID(❶·H1·T2·결함번호·severity 코드 등) 노출 0 / 일상 비유 ≥1 / 격식 기술 보고서 말투 금지.

## 응답 스타일 정합 (HARD ENFORCE — 산문 톤 게이트)

> 산문 톤은 길이 한계와 **동급 게이트**. 위반 시 user-friendly-reporter 재호출 (max 2회).

`communication-style.md` 15세 기준 체크리스트:
- [ ] **비유 ≥1개**: Hook 또는 본문에 일상 비유 (책·집·요리 등). 통계·대조만으로는 부족.
- [ ] **약어·전문어 첫 등장 풀이**: 정본 / drift / critic / applier / AC / SSOT / Flag chain 등 → 쉬운 말 병기.
- [ ] **내부 ID 노출 0**: ❶❷·H1·T2·결함번호·severity 코드 → 일상어 설명으로 대체.
- [ ] **격식 기술 보고서 말투 금지**: 원본 PRD 문장 복사 = 위반 (작성 원칙 #3 참조).
- [ ] 다이어그램 우선 + 한 줄 요약 마지막.

> **근본 원인 메모 (2026-06-05)**: `aiden-auto-charter.summary.html` 이 원본 PRD 전문어를 그대로 복사해 톤 위반. 작성 원칙 #3 "추출"이 산문에까지 잘못 적용된 함정 → 본 섹션 HARD ENFORCE 승격으로 차단.
