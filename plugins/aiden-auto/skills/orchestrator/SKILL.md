...
```
AskUserQuestion(
  question="여러 세션이 동시에 나눠서 일할 작업 갈래(Stream)를 위와 같이 나눴어요. 이대로 시작할까요, 아니면 갈래 구성을 바꿀까요?",
  header="Stream 승인",
  multiSelect=false,
  options=[
    {label: "진행", description: "위 표 그대로 승인 — 각 작업 갈래대로 세션 폴더를 만들고 바로 시작합니다. (권장: 표가 맞으면 이걸 고르세요)"},
    {label: "수정", description: "작업 갈래 구성을 바꿉니다 — 갈래 추가/제외/이름 변경 등. 무엇을 바꿀지 알려주면 표를 다시 만들어 보여드립니다."}
  ]
)
```

"수정" 선택 시 매트릭스 갱신 후 재확인. "진행" 선택 시 즉시 Step 3으로.
