...
- `working_directory_context` (필수): 파일 검증 시 정본 경로 명시. 추측 금지.

## Working Directory 정합 룰 (CRITICAL — false positive 방지)

파일 존재 여부 검증 시 **반드시 다음 경로 우선순위**로 확인:

| 우선순위 | 경로 | 용도 |
|:--------:|------|------|
| 1 | `C:\Users\AidenKim\.claude\` | 정본 SSOT (user-global) |
| 2 | `C:\Users\AidenKim\.claude\projects\{project}\memory\` | 사용자 메모리 |
| 3 | `C:\Users\AidenKim\.claude\references\` | 참조 문서 |
| 4 | `C:\Users\AidenKim\.claude\agents\`, `\skills\`, `\hooks\`, `\commands\` | 핵심 entity |
| 5 | `C:\claude\.claude\` | project mirror (검증 X, 정본은 1) |
| 6 | `C:/claude/plugins/aiden-auto/` | plugin mirror (검증 X, 정본은 1) |

**금지**:
- `C:\claude\.claude\memory\` 와 같은 mirror 경로에서 "파일 부재" 보고 — 정본 1번에서 다시 확인 필수
- `\phases\` 같은 추측 디렉토리 이름 — 실제는 `\references\` 또는 `\agents\` 등
- working directory 미확인 상태에서 "파일 부재" 보고

**호출자 (Lead) 의무**:
- critic prompt에 정본 경로 명시 (예: "정본 경로: ~/.claude/agents/core/, ~/.claude/references/")
- 파일 경로 인용 시 `C:\Users\AidenKim\.claude\` 형식 사용
